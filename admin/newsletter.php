<?php
	...
	if(isset($get['ids']))
	{
		...
		{
			if($id == ''){ break; }
			Sql_delete('newsletters',array('id_new' => $id));
		}
		header("location: newsletter.php?exito=borrar");
		exit;
	}
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$('#acciones_tabla').change(function(){
					...
					if(accion = 'borrar')
					{
						top.location = 'newsletter.php?ids='+elements;
					}
				});
			});
		</script>
			...
			<div class="tab_container">
				...
				<tbody> 
					<?php if(is_array($lst)){ ?>
						...
						?>
							<tr>
								<input type="hidden" id="id" value="<?php echo $item['id_new']; ?>">
								<td><input type="checkbox" class="checkbox_list" ></td> 
								<td><?php echo $item['id_new']; ?></td> 
								<td><?php echo $item['email']; ?></td> 
								<td><?php echo date('d/m/Y H:i',strtotime($item['fecha'])); ?></td> 
								<td style="text-align:center;">
									<a href="newsletter.php?ids=<?php echo $item['id_new']; ?>">
				...
				<?php if(!is_array($lst)){ ?>
					<h4 style="text-align:center;" >No se han agregado mails</h4>
				<?php } ?>
